...
domainSize_LIST = "128 144 160 176 192 208 224 240 256 272 288 304 320 336 352 368 384 400 416 432 448 464 480 496 512".split()
computeFP_LIST = "double float".split()
dimSpace_LIST = "2D 3D".split()
...
with (open(command + '.log', 'w') as fp):
    ...
    while len(benchmarks_compressed) > 0:
        ...
        while len(benchmarks_expanded) > 0:
            ...
            parameters.append('--report-filename ' + 'seven')
            parameters.append('--computeFP ' + benchmark['computeFP'])
            parameters.append('--dimSpace ' + benchmark['dimSpace'])
