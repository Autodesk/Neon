...
class Vec_2d<RealType_ta, false, true>
{
   public:
    using eValue_t = RealType_ta;
    using self_t = Vec_2d<eValue_t, false, true>;
    ...
    union
    {
        eValue_t v[axis_e::num_axis]{
            static_cast<eValue_t>(.0), static_cast<eValue_t>(.0)};
        struct
        {
            union
            {
    ...
    /**
     * All component of the 2d tuple are set to the same scalar value.
    ...
    /**
     * Creates a 2d tuple with specific values for each component.
     ...
     */
    NEON_CUDA_HOST_DEVICE inline Vec_2d(const self_t& xyz);

    NEON_CUDA_HOST_DEVICE inline Vec_2d& operator=(const eValue_t other);

    NEON_CUDA_HOST_DEVICE inline Vec_2d& operator=(const self_t& other);

    NEON_CUDA_HOST_DEVICE inline Vec_2d& operator=(const eValue_t other[axis_e::num_axis]);

    NEON_CUDA_HOST_DEVICE inline void set(const eValue_t val);

    NEON_CUDA_HOST_DEVICE inline void set(const eValue_t px, const eValue_t py);

    NEON_CUDA_HOST_DEVICE inline void set(const eValue_t p[axis_e::num_axis]);

    NEON_CUDA_HOST_DEVICE inline void set(const self_t& xy);

    //---- [REDUCE SECTION] --------------------------------------------------------------------------------------------
    //---- [REDUCE SECTION] --------------------------------------------------------------------------------------------
    //---- [REDUCE SECTION] --------------------------------------------------------------------------------------------
    /**
     *   Extracts the max value stored by the 2d tuple.
     *   @return max value
     */
    NEON_CUDA_HOST_DEVICE inline eValue_t rMax() const;

    /**
     *   Extracts the min value stored by the 2d tuple.
     *   @return min value.
     */
    NEON_CUDA_HOST_DEVICE inline eValue_t rMin() const;

    /**
     *   Extracts the max absolute value stored by the 2d tuple.
    ...
    /**
     * Returns a 2d tuple where each component is the absolute value of the associated input component: {x,y,z}.abs() -> {|x|, |y|, |z|}
     * @return {|x|, |y|, |z|}
     * */
    inline self_t abs() const;

    /**
     * Returns a 2d tuple where each component is the power of 2 of the associated input component: {x,y,z}.getPow2() -> {x^2, y^2, z^2}
     * @return {x^2, y^2, z^2}
     */
    ...
    /**
     * return the norm ov the 2d tuple.
     * @return {x,y,z}.norm() -> {x,y,z}
     * */
    NEON_CUDA_HOST_DEVICE inline eValue_t normSq() const;

    /**
     * return the norm ov the 2d tuple.
     * @return {x,y,z}.norm() -> |{x,y,z}|
     * */
    inline NEON_CUDA_HOST_DEVICE eValue_t norm() const;

    /**
     * Returns the normalized vector of the 2d tuple.
     * @return normalized 2d tuple.
     */
    ...
    //---- [Index SECTION] ----------------------------------------------------------------------------------------------
    /**
     * Returns the index of 2d tuple element with the higher value.
     * @return component index (0, 1 or 2)
     */
    NEON_CUDA_HOST_DEVICE inline index_t idxOfMax() const;

    /**
     * Returns the index of 2d tuple element with the lower value.
     * @return component index (0, 1 or 2)
     */
    NEON_CUDA_HOST_DEVICE inline index_t idxOfMin() const;

    /**
     * Returns a mask which has a non zero element only for the 2d tuple component that stores the max value.
     * @return component index (0, 1 or 2)
     */
    NEON_CUDA_HOST_DEVICE inline Vec_2d<index_t> idxMinMask() const;
    /**
     * Returns a mask. The mask is computed by ordering the 2d tuple component values from higher to lower.
     * @return component index (0, 1 or 2)
     */
    ...
    NEON_CUDA_HOST_DEVICE inline self_t operator*(const eValue_t& alpha) const;
    ...
    NEON_CUDA_HOST_DEVICE static eValue_t dot(const self_t& a, const self_t& b);
};


template <typename T_ta>
std::ostream& operator<<(std::ostream& out, const Real_2d<T_ta>& p);
