    ...
    return in_voxels.getGrid().newContainer(
        "ComputeVelocity", [&, tau](Neon::set::Loader& loader) {
            ...
            return [=] NEON_CUDA_HOST_DEVICE(
                ...
                int mask_val = m.getNghData(idx, ngh, 0, default_value).value();
                for (int k = 0; k < COMP; k++) {
                    T f = ins.getNghData(idx, ngh, k, default_value).value();
                    olds(idx, k) = f;
                    r += f;
                    for (int i = 0; i < DIM; i++) {
                        T e_i = get_e<DIM>(k, i);
        ...
        "CollideAndStream", [&, tau](Neon::set::Loader& loader) {
            ...
            return [=] NEON_CUDA_HOST_DEVICE(
                ...
                for (int k = 0; k < COMP; k++) {
                    ...
                    ngh.y = -get_e<DIM>(k, 1);
                    ngh.z = -get_e<DIM>(k, 2);
                    T fold = ins.getNghData(idx, ngh, k, default_value).value();
                    T r = rho.getNghData(idx, ngh, 0, default_value).value();
                    for (int i = 0; i < DIM; i++) {
                        int e_i = get_e<DIM>(k, i);
                        vel = in_vel.getNghData(idx, ngh, i, default_value).value();
                        eu += (e_i * vel);
                        uv += (vel * vel);
                    }
    ...
    return in_voxels.getGrid().newContainer(
        "BoundaryConditions", [&, tau](Neon::set::Loader& loader) {
            ...
            return [=] NEON_CUDA_HOST_DEVICE(
                ...
                int boundary = mask.getNghData(idx, ngh, 0, default_value).value();
                for (int i = 0; i < DIM; i++) {
                    vels[i] = in_vel.getNghData(idx, ngh, i, default_value).value();
                }
                ...
                T r = rho_old.getNghData(idx, ngh, 0, default_value).value();
                if (boundary == 1) {  // fixed boundary
                    ...
                    for (int i = 0; i < DIM; i++) {
                        new_vals[i] = in_vel.getNghData(idx, ngh, i, default_value).value();
                        out_vel(idx, i) = new_vals[i];
                    }
