        ...
        const -> NghData<TypeT>
{
    ...
    {  // STEP 1
        ...
        if (!isActive) {
            return NghData(alternativeVal, false);
        }
    }
    return mBuildingBlockPartition.template getNghData < sx == 1 ? 0 : sx,
           sy == 1 ? 0 : sy,
        ...
        const -> NghData<TypeT>
{
    ...
    return mBuildingBlockPartition.getNghData(node.getBuildingBlockCell(),
                                              xyz,
                                              cardinalityIdx,
                                              alternativeVal);
}
