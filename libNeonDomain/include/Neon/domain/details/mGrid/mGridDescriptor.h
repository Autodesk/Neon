...
struct mGridDescriptor
{
     ...
     * @param parentLevel the level at which the parent resides
     */
    auto child1DIndex(const Neon::index_3d& localChild, int parentLevel) const -> int64_t
    {
        const int parentRefFactor = getRefFactor(parentLevel);

        return static_cast<int64_t>(localChild.x) +
               static_cast<int64_t>(localChild.y) * parentRefFactor +
               static_cast<int64_t>(localChild.z) * parentRefFactor * parentRefFactor;
    }
     ...
     * @param localChild child local index within the parent block
     */
    auto flattened1DIndex(const Neon::index_3d& parentID,
                          const int             parentLevel,
                          const Neon::index_3d& parenLevelNumBlocks,
                          const Neon::index_3d& localChild) const -> int64_t
    {
        const int parentRefFactor = getRefFactor(parentLevel);

        int64_t parentBlockID = static_cast<int64_t>(parentID.x) +
                                static_cast<int64_t>(parentID.y) * parenLevelNumBlocks.x +
                                static_cast<int64_t>(parentID.z) * parenLevelNumBlocks.x * parenLevelNumBlocks.y;

        const int64_t childID = child1DIndex(localChild, parentLevel) +
