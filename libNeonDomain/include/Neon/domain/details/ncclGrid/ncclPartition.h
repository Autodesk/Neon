...
{
   public:
    using PartitionIndexSpace = ncclSpan;
    using Span = ncclSpan;
    using Self = ncclPartition<T, C>;
    using Idx = ncclIndex;
   ...
   public:
    ncclPartition() = default;

    ~ncclPartition() = default;

    explicit ncclPartition(Neon::DataView dataView,
                        T*             mem,
                        Neon::index_3d dim,
                        int            zHaloRadius,
                        int            zBoundaryRadius,
    ...
    auto ioToVti(std::string const& fname, std::string const& fieldName)
    {
        ...
        io.addField([&](const Neon::index_3d& idx, int i) {
            return operator()(ncclIndex(idx), i);
        },
