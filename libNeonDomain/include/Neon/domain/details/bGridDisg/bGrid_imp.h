...
bGridDisg<SBlock>::bGridDisg(const Neon::Backend&                         backend,
    ...
    : bGridDisg(backend, domainSize, activeCellLambda, stencil, 1, spacingData, origin, encoderType)
{
}
                     ...
                     Neon::domain::tool::spaceCurves::EncoderType encoderType)
{
             ...
             << SBlock::memBlockSizeZ;
    {
        ...
        bGridDisg::GridBase::init(gridName.str(),
                              backend,
                              domainSize,
                              stencil,
    ...
    {  // Initialization of the partitioner
        ...
        if constexpr (std::is_same_v<returTypeOfLambda, bool>) {
            mData->partitioner1D = Neon::domain::tool::Partitioner1D(
                ...
                encoderType,
                multiResDiscreteIdxSpacing);
        } else if constexpr (std::is_same_v<returTypeOfLambda, Neon::ClassSelector>) {
            mData->partitioner1D = Neon::domain::tool::Partitioner1D(
                backend,
                [&](Neon::index_3d idx) {
                    return activeCellLambda(idx) != Neon::outside;
                },
                [&](Neon::index_3d idx) {
                    return (activeCellLambda(idx) == Neon::beta)
...
#pragma omp critical
    ...
    mData->alphaGrid = details::cGrid::cGrid<SBlock, int(Neon::alpha)>(*this);
    mData->betaGrid = details::cGrid::cGrid<SBlock, int(Neon::beta)>(*this);
    init_mask_field(activeCellLambda);
}

template <typename SBlock>
template <typename T, int C>
auto bGridDisg<SBlock>::newField(const std::string   name,
                             int                 cardinality,
                             T                   inactiveValue,
...
template <typename SBlock>
template <typename T, int C>
auto bGridDisg<SBlock>::newBlockViewField(const std::string   name,
                                      int                 cardinality,
                                      T                   inactiveValue,
                                      Neon::DataUse       dataUse,
...
auto bGridDisg<SBlock>::newContainer(const std::string& name,
                                 index_3d           blockSize,
                                 size_t             sharedMem,
...
template <Neon::Execution execution,
          typename LoadingLambda>
auto bGridDisg<SBlock>::newAlphaContainer(const std::string& name,
                                      LoadingLambda      lambda) const -> Neon::set::Container
{
...
template <Neon::Execution execution,
          typename LoadingLambda>
auto bGridDisg<SBlock>::newBetaContainer(const std::string& name,
                                     LoadingLambda      lambda) const -> Neon::set::Container
{
...
auto bGridDisg<SBlock>::newAlphaBetaContainer(const std::string& name,
...
template <typename SBlock>
template <int dummy>
auto bGridDisg<SBlock>::helGetMultiResDiscreteIdxSpacing() const
...
template <typename SBlock>
auto bGridDisg<SBlock>::getSpan(Neon::Execution execution,
                            SetIdx          setIdx,
                            Neon::DataView  dataView) -> const bGridDisg::Span&
{
    return mData->spanTable.getSpan(execution, setIdx, dataView);
}

template <typename SBlock>
bGridDisg<SBlock>::~bGridDisg()
{
}
template <typename SBlock>
auto bGridDisg<SBlock>::getSetIdx(const index_3d& idx) const -> int32_t
{
...
template <typename SBlock>
auto bGridDisg<SBlock>::getLaunchParameters(Neon::DataView dataView,
                                        const index_3d&,
...
template <typename SBlock>
auto bGridDisg<SBlock>::isInsideDomain(const index_3d& idx) const -> bool
{
...
template <typename SBlock>
auto bGridDisg<SBlock>::getProperties(const index_3d& idx)
...
template <typename SBlock>
auto bGridDisg<SBlock>::helpGetSetIdxAndGridIdx(Neon::index_3d idx)
...
template <typename SBlock>
auto bGridDisg<SBlock>::helpGetPartitioner1D() -> Neon::domain::tool::Partitioner1D&
{
...
template <typename SBlock>
template <typename ActiveCellLambda>
auto bGridDisg<SBlock>::init_mask_field([[maybe_unused]] ActiveCellLambda activeCellLambda) -> void
{
    using returTypeOfLambda = typename std::invoke_result<ActiveCellLambda, Neon::index_3d>::type;
...
template <typename SBlock>
auto bGridDisg<SBlock>::helpGetClassField() -> Field<uint8_t, 1>&
{
