...
bDisgPartition<T, C, SBlock>::bDisgPartition()
    : mCardinality(0),
      mMem(nullptr),
      mStencilNghIndex(),
...
template <typename T, int C, typename SBlock>
NEON_CUDA_HOST_DEVICE inline auto bDisgPartition<T, C, SBlock>::
...
template <typename T, int C, typename SBlock>
inline NEON_CUDA_HOST_DEVICE auto bDisgPartition<T, C, SBlock>::
    cardinality()
        const -> int
{
    return mCardinality;
...
{
    NghData result;
    bDisgIndex  nghIdx = helpGetNghIdx(idx, offset);
...
{
    NghData result;
    bDisgIndex  nghIdx = helpGetNghIdx<xOff, yOff, zOff>(idx);
    auto [isValid, pitch] = helpNghPitch(nghIdx, card);
    if (!isValid) {
        result.invalidate();
        return result;
...
{
    NghData result;
    bDisgIndex  nghIdx = helpGetNghIdx<xOff, yOff, zOff>(gidx);
    auto [isValid, pitch] = helpNghPitch(nghIdx, card);
...
bDisgPartition<T, C, SBlock>::isActive(const Idx&                      cell,
